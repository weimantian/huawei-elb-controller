...
> ⚠️ **重要**：不要填写 `kubernetes.io/elb.id`。绑定已有 ELB 会导致端口冲突（primary 和 replicas 两个 Service 绑定同一个 ELB 的 3306 端口）。控制器会跳过任何含 `kubernetes.io/elb.id` 的 LBC。请使用 `huawei-elb.io/*` 注解代替。

> **注意**：`eip-type` 创建后不可变更（华为云 API 限制），修改此参数需删除重建 ELB。

> ⚠️ **CCE 不可变注解**：ELB 绑定后，CCE 将以下注解视为不可变，通过 PXC operator 修改会失败：`kubernetes.io/elb.class`、`kubernetes.io/elb.id`（均由 CCM 写入）。如需修改带宽或计费模式，请直接 `kubectl annotate svc` 到 Service，而不是修改 LBC。
### ACL 注解
