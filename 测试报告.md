...
| 子网 ID | `c265b187-a0a8-45cf-9cb3-7c3b757f8ff8` |
| 可用区 | `cn-north-4a` |
...
spec:
  annotations:
    ...
    kubernetes.io/elb.id: 27b11f08-8b57-48fb-a654-ce616818e410
metadata:
  annotations:
...
postgresql-j00-pgbouncer   LoadBalancer   10.247.79.190   192.168.0.107   5432:31923/TCP
```
...
postgresql-j00   4      4       ready    192.168.0.107   13m
```
...
*图 13: 数据库概览页面 — Host: 192.168.0.107，Connection URL 已生成*
...
```bash
$ nc -zv 192.168.0.107 5432
192.168.0.107 (192.168.0.107:5432) open
...
```bash
$ psql -h 192.168.0.107 -p 5432 -U postgres -d postgres -c 'SELECT version();'
...
| ELB 绑定 Service | ✅ 通过 | Service EXTERNAL-IP: 192.168.0.107 |
| TCP 连接 | ✅ 通过 | 192.168.0.107:5432 可达 |
